...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:my_choice) {described_class.new("my_type")}
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(my_choice.type).to eq("my_type")
      end
  ...
  describe '#full?' do
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:my_choice) {described_class.new("my_type",15)}
      # remove the 'x' before running this test
      it 'is not full' do
        expect(my_choice).not_to be_full
      end
    end
  end
end
